...
// File: idiom/pct_profiler.cpp - Implementation of the controller
// for idiom PCT profiler.

#include "idiom/pct_profiler.hpp"

namespace idiom {

PCTProfiler::PCTProfiler()
    : iroot_db_(NULL),
      memo_(NULL),
      sinst_db_(NULL),
...
void PCTProfiler::HandlePreSetup() {
  pct::Scheduler::HandlePreSetup();
  ...
  predictor_ = new Predictor(knob_);
  predictor_new_ = new PredictorNew(knob_);
...
void PCTProfiler::HandlePostSetup() {
  pct::Scheduler::HandlePostSetup();
...
bool PCTProfiler::HandleIgnoreInstCount(IMG img) {
...
bool PCTProfiler::HandleIgnoreMemAccess(IMG img) {
  if (!IMG_Valid(img))
    return true;
  Image *image = sinfo_->FindImage(IMG_Name(img));
  DEBUG_ASSERT(image);
