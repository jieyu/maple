...
// File: idiom/randsched_profiler.cpp - Implementation of the controller
// for idiom Random profiler.

#include "idiom/randsched_profiler.hpp"

namespace idiom {

RandSchedProfiler::RandSchedProfiler()
    : iroot_db_(NULL),
      memo_(NULL),
      sinst_db_(NULL),
...
bool RandSchedProfiler::HandleIgnoreInstCount(IMG img) {
...
bool RandSchedProfiler::HandleIgnoreMemAccess(IMG img) {
  if (!IMG_Valid(img))
    return true;
  Image *image = sinfo_->FindImage(IMG_Name(img));
  DEBUG_ASSERT(image);
